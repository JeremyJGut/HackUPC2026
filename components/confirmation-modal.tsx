...
export function ConfirmationModal({ pending, onClose, onConfirm }: ConfirmationModalProps) {
  const focusPointId = pending.after.headId;
  const details = [
    {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-50 bg-slate-950/78 p-3 backdrop-blur-sm sm:p-4"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      <motion.div
        className="flex h-full w-full flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/95 p-4 shadow-2xl shadow-cyan-500/10 sm:p-6"
            ...
            <div className="grid min-h-0 gap-4">
              <div className="rounded-[1.75rem] border border-white/10 bg-black/20 p-4">
                ...
                <div className="min-h-[320px] lg:min-h-[360px]">
                  <GitTreeVisualizer repository={pending.before} compact focusPointId={focusPointId} />
                </div>
              </div>
              ...
              <div className="rounded-[1.75rem] border border-cyan-400/20 bg-cyan-400/5 p-4">
                ...
                <div className="min-h-[320px] lg:min-h-[360px]">
                  <GitTreeVisualizer repository={pending.after} compact focusPointId={focusPointId} />
                </div>
              </div>
            </div>
          </div>
